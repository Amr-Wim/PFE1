...
<main class="container">
        <h2 class="text-center soins-title">Liste des Observations</h2>
        <div class="search-bar">
            <form method="get" action="${pageContext.request.contextPath}/RechercherObservationServlet">
        ...
        <c:if test="${not empty idPatientRecherche}">
            <div class="card p-4 mt-4">
                ...
                <c:choose>
                    <c:when test="${not empty observations}">
                        <table class="table table-striped">
                            <thead>
                                <tr>
                                    <th>ID</th><th>Température</th><th>Tension</th><th>Fréq. Cardiaque</th>
                                    <th>Observation</th><th>Date</th><th>Infirmier</th>
                                </tr>
                            </thead>
                            <tbody>
                                <c:forEach var="obs" items="${observations}">
                                    <tr>
                                        <td>${obs.id}</td><td>${obs.temperature}°C</td><td>${obs.tension}</td>
                                        <td>${obs.frequence} bpm</td><td><c:out value="${obs.texte}"/></td>
                                        <td><fmt:formatDate value="${obs.date}" pattern="dd/MM/yyyy HH:mm"/></td>
                                        <td><c:out value="${obs.nomInfirmier}"/></td>
                                    </tr>
                                </c:forEach>
                            </tbody>
                        </table>
                    ...
                    <c:otherwise>
                        <div class="alert alert-warning">Aucune observation trouvée pour ce patient.</div>
                    </c:otherwise>
                </c:choose>
            </div>
        </c:if>
